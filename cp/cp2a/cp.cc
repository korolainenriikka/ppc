...
void correlate(int ny, int nx, const float *data, float *result) {
    ...
    for (int y = 0; y < ny; ++y) {
        for (int x = y+1; x < ny; ++x) {
            ...
            //std::cout << x;
            //yth row and xth row
            double dot_product_even = 0.0;
            double dot_product_odd = 0.0;
            for (int k = 0; k < nx; k += 2) {
                // rows i and j in index k
                //std::cout << normalized[k + y*nx] << " " << normalized[k + x*nx] << "\n";
                dot_product_even += normalized[k + y*nx] * normalized[k + x*nx];
                if (k + 1 < nx) {
                    dot_product_odd += normalized[k + y*nx + 1] * normalized[k + x*nx + 1];
                }
            }
            //std::cout <<dot_product;
            result[x + y * ny] = dot_product_even + dot_product_odd;
        }
    }
