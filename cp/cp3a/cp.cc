...
void correlate(int ny, int nx, const float *data, float *result) {
    ...
    for (int y = 0; y < count_of_squares_y * 3; y += 3) {
        for (int x = (std::floor(y/3) + 1) * 3; x < count_of_squares_x * 3; x += 3) {
            ...
            for (int k = 0; k < na; ++k) {
                double4_t y0 = vnorm[na*(y + 0) + k];
                double4_t y1 = vnorm[na*(y + 1) + k];
                double4_t y2 = vnorm[na*(y + 2) + k];
                double4_t x0 = vnorm[na*(x + 0) + k];
                double4_t x1 = vnorm[na*(x + 1) + k];
                double4_t x2 = vnorm[na*(x + 2) + k];
                dp_vecs[0][0] += y0 * x0;
                dp_vecs[0][1] += y0 * x1;
                dp_vecs[0][2] += y0 * x2;
                dp_vecs[1][0] += y1 * x0;
    ...
    for (int y = 0; y < ny; ++y) {
        for (int x_leftover = 0; x_leftover < no_of_leftover_cols; ++x_leftover) {
            ...
            double4_t dot_product_v = zeros;
            for (int k = 0; k < na; ++k) {
                double4_t tulot = vnorm[k + y*na] * vnorm[k + x*na];
                dot_product_v += tulot;
            }
            double dot_product = vec_sum(dot_product_v);
